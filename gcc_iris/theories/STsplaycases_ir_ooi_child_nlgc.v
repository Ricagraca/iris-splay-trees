...
Section SplaySubCasesRightLeftRotation.
  ...
  Lemma splay_root_rotate_right_left_ngc_st F FF (pp p p' y x : loc) (z : Z) :
    Inv p D F V W ->
    Mem D F V M ->
  ...
  Proof.
    ...
    + destruct (M !! y) eqn:E2 ; try contradiction.
      ...
      iPoseProof (rotate_left_child_BBN_r_st D V W F M pp p l x y l1
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeB (V p) l y]> (<[x:=NodeR (V x) l1]> (<[y:=NodeR (V y) x]> M))) as Mp'.
      ...
      iExists (<[p:=NodeL (V p) l]>
               (<[y:=NodeB (V y) p x]>
                (<[p:=NodeB (V p) l y]> (<[x:=NodeR (V x) l1]> (<[y:=NodeR (V y) x]> M))))) ; try iFrame. auto.
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_left_child_BLN_r_st D V W F M pp p l x y 
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeB (V p) l y]> (<[x:=NodeN (V x)]> (<[y:=NodeR (V y) x]> M))) as Mp'.
      assert (Mp' !! p = Some (NodeB (V p) l y)).
      { rewrite HeqMp'. rewrite lookup_insert. auto. }
      assert (Mp' !! y = Some (NodeR (V y) x)).
      { rewrite HeqMp'. do 2(rewrite lookup_insert_ne ; stlink_tac). rewrite lookup_insert. auto. }
      ...
      iExists (<[p:=NodeL (V p) l]>
               (<[y:=NodeB (V y) p x]>
                (<[p:=NodeB (V p) l y]> (<[x:=NodeN (V x)]> (<[y:=NodeR (V y) x]> M))))). iFrame.
      auto.    
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_left_child_RBN_r_st D V W F M pp p x y l0
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeR (V p) y]> (<[x:=NodeR (V x) l0]> (<[y:=NodeR (V y) x]> M))) as Mp'.
      ...
      iExists (<[p:=NodeN (V p)]>
               (<[y:=NodeB (V y) p x]>
                (<[p:=NodeR (V p) y]> (<[x:=NodeR (V x) l0]> (<[y:=NodeR (V y) x]> M))))). iFrame.
      unpack. iSplit ; iPureIntro ; auto.    
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_left_child_RLN_r_st D V W F M pp p x y
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeR (V p) y]> (<[x:=NodeN (V x)]> (<[y:=NodeR (V y) x]> M))) as Mp'.
      assert (Mp' !! p = Some (NodeR (V p) y)).
      { rewrite HeqMp'. rewrite lookup_insert. auto. }
      assert (Mp' !! y = Some (NodeR (V y) x)).
      { rewrite HeqMp'. do 2(rewrite lookup_insert_ne ; stlink_tac). rewrite lookup_insert. auto. }
      ...
      iExists (<[p:=NodeN (V p)]>
               (<[y:=NodeB (V y) p x]>
                (<[p:=NodeR (V p) y]> (<[x:=NodeN (V x)]> (<[y:=NodeR (V y) x]> M))))). iFrame.
      auto.
  Qed.
...
Section SplaySubCasesLeftRightRotation.
  ...
  Lemma splay_root_rotate_left_right_ngc_st F FF (pp p p' y x : loc) (z : Z) :
    Inv p D F V W ->
    Mem D F V M ->
  ...
  Proof.
    ...
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_right_child_BBN_l_st D V W F M pp p x l0 l y
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeB (V p) y l0]> (<[x:=NodeL (V x) l]> (<[y:=NodeL (V y) x]> M))) as Mp'.
      ...
      iExists (<[p:=NodeR (V p) l0]>
               (<[y:=NodeB (V y) x p]>
                (<[p:=NodeB (V p) y l0]> (<[x:=NodeL (V x) l]> (<[y:=NodeL (V y) x]> M))))). iFrame.
      unpack. iSplit ; iPureIntro ; auto.
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_right_child_BRN_l_st D V W F M pp p x l0 y
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeB (V p) y l0]> (<[x:=NodeN (V x)]> (<[y:=NodeL (V y) x]> M))) as Mp'.
      assert (Mp' !! p = Some (NodeB (V p) y l0)).
      { rewrite HeqMp'. rewrite lookup_insert. auto. }
      assert (Mp' !! y = Some (NodeL (V y) x)).
      { rewrite HeqMp'. do 2(rewrite lookup_insert_ne ; stlink_tac). rewrite lookup_insert. auto. }
      ...
      iExists (<[p:=NodeR (V p) l0]>
               (<[y:=NodeB (V y) x p]>
                (<[p:=NodeB (V p) y l0]> (<[x:=NodeN (V x)]> (<[y:=NodeL (V y) x]> M))))). iFrame.
      unpack. iSplit ; iPureIntro ; auto.
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_right_child_LBN_l_st D V W F M pp p x l y
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeL (V p) y]> (<[x:=NodeL (V x) l]> (<[y:=NodeL (V y) x]> M))) as Mp'.
      ...
      iExists (<[p:=NodeN (V p)]>
               (<[y:=NodeB (V y) x p]>
                (<[p:=NodeL (V p) y]> (<[x:=NodeL (V x) l]> (<[y:=NodeL (V y) x]> M))))). iFrame.
      unpack. iSplit ; iPureIntro ; auto.
    + destruct (M !! y) eqn:E2 ; try contradiction.
      destruct c ; unpack ; subst ; [exfalso;eauto|exfalso;eauto|exfalso;eauto|].
      iPoseProof (rotate_right_child_LRN_l_st D V W F M pp p x y
                                             (V p) (V x) (V y)) as "Hf" ; auto.
      iApply ("Hf" with "[R M]") ; iFrame ; auto. iClear "Hf".
      iModIntro. iIntros "[Hpp [% [% M]]]". wp_seq.
      remember (<[p:=NodeL (V p) y]> (<[x:=NodeN (V x)]> (<[y:=NodeL (V y) x]> M))) as Mp'.
      assert (Mp' !! p = Some (NodeL (V p) y)).
      { rewrite HeqMp'. rewrite lookup_insert. auto. }
      assert (Mp' !! y = Some (NodeL (V y) x)).
      { rewrite HeqMp'. do 2(rewrite lookup_insert_ne ; stlink_tac). rewrite lookup_insert. auto. }
      ...
      iExists (<[p:=NodeN (V p)]>
               (<[y:=NodeB (V y) x p]>
                (<[p:=NodeL (V p) y]> (<[x:=NodeN (V x)]> (<[y:=NodeL (V y) x]> M))))). iFrame.
...
Section SplaySubCasesOO'Rotation.
  ...
  Lemma splay_root_rotate_o_o'_ngc_st F FF (pp p p' y x : loc) (z : Z) o :
    Inv p D F V W ->
    Mem D F V M ->
  ...
  Proof.
    destruct o.
    + apply splay_root_rotate_left_right_ngc_st ; auto.
    + apply splay_root_rotate_right_left_ngc_st ; auto.
  Qed.
