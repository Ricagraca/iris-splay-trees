...
Section SplaySubCasesLeftGCRotation.
  ...
  Lemma splay_root_rotate_left_gc_eq_st F (pp p p' y : loc) (z : Z) (n : nat) :
    Inv p D F V W ->
    Mem D F V M ->
    z < V p -> 
    F p p' LEFT ->
    z = V p' -> 
    F p' y RIGHT ->
    let F' := (update_edge F p' y p RIGHT) in
    let F'' := (update_edge F' p p' y LEFT) in
    {{{ pp ↦ #p ∗ mapsto_M M }}}
  ...
  Proof.
    ...
    - iPoseProof
        (rotate_left_BB_st' D V W F M pp p p' l0 l1 l2 (V p) (V p') with "[R M]")
        as "HF" ; auto.
      iFrame ; iSplit ; iPureIntro ; auto.
      wp_bind (rotate_left #pp #p #p').
      iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
      iApply "H" ; iFrame. assert (y = l2). stlink_tac. subst. auto.
    - iPoseProof
        (rotate_left_BR_st' D V W F M pp p p' l0 l1 (V p) (V p') with "[R M]")
        as "HF" ; auto.
      iFrame ; iSplit ; iPureIntro ; auto.
      wp_bind (rotate_left #pp #p #p').
      iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
      iApply "H" ; iFrame. assert (y = l1). stlink_tac. subst. auto.
    + assert (l \in D) ; stdomain_tac ;
        apply H0 in H10 ;
      ...
      [| exfalso;eauto | | exfalso; eauto ].
    - iPoseProof
        (rotate_left_LB_st' D V W F M pp p p' l0 l1 (V p) (V p') with "[R M]")
        as "HF" ; auto.
      iFrame ; iSplit ; iPureIntro ; auto.
      wp_bind (rotate_left #pp #p #p').
      iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
      iApply "H" ; iFrame. assert (y = l1). stlink_tac. subst. auto.
    - iPoseProof
        (rotate_left_LR_st' D V W F M pp p p' l0 (V p) (V p') with "[R M]")
        as "HF" ; auto.
      iFrame ; iSplit ; iPureIntro ; auto.
      wp_bind (rotate_left #pp #p #p').
      iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
      iApply "H" ; iFrame. assert (y = l0). stlink_tac. subst. auto.
  Qed.

  Lemma splay_root_rotate_left_gc_gt_ne_st F (pp p p' y : loc) (z : Z) (n : nat) :
    Inv p D F V W ->
    Mem D F V M ->
    z < V p -> 
    F p p' LEFT ->
                ...
                assert (l = p') ; stlink_tac ; subst ;
                  ...
                  iModIntro ; iIntros ; wp_pures ;
                      ...
                      iDestruct "M" as (v') "[% [Hp M]]" ; inversion H16 ; subst ;
                        ...
                        iApply (left_child_specification with "[Hp]") ; auto ;
                          iModIntro ; iIntros "Hp" ;
                            iDestruct ("M" with "Hp") as "M" ; wp_pures ; [exfalso;eauto|].
          - iPoseProof
              (rotate_left_BR_st' D V W F M pp p p' l0 l1 (V p) (V p') with "[R M]") as "HF" ; auto.
            iFrame ; iSplit ; iPureIntro ; auto.
            wp_bind (rotate_left #pp #p #p').
            iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
                      ...
                      iDestruct "M" as (v') "[% [Hp M]]" ; inversion H16 ; subst ;
                        ...
                        iApply (left_child_specification with "[Hp]") ; auto ;
                          iModIntro ; iIntros "Hp" ;
                            iDestruct ("M" with "Hp") as "M" ; wp_pures ; [exfalso;eauto|].
          - iPoseProof
              (rotate_left_LR_st' D V W F M pp p p' l0 (V p) (V p') with "[R M]") as "HF" ; auto.
            iFrame ; iSplit ; iPureIntro ; auto.
            wp_bind (rotate_left #pp #p #p').
            iApply "HF". iModIntro. iIntros "[HPP [% [% M]]]". wp_seq.
            assert (l0 = y). stlink_tac. subst.
            iApply "H" ; iFrame. iSplit ; iPureIntro ; auto.
  Qed.
  
  Lemma splay_root_rotate_left_gc_st F (pp p p' y : loc) (z : Z) (n : nat) :
    Inv p D F V W ->
    Mem D F V M ->
    z < V p -> 
    F p p' LEFT ->
    (z = V p') \/ (z < V p')%Z ∧ ¬ (exists t, F p' t LEFT) ->
    F p' y RIGHT ->
    let F' := (update_edge F p' y p RIGHT ) in
    let F'' := (update_edge F' p p' y LEFT) in
    {{{ pp ↦ #p ∗ mapsto_M M }}}
  ...
  Proof.
    intros.
    destruct H3.
    - apply splay_root_rotate_left_gc_eq_st ; auto.
    - apply splay_root_rotate_left_gc_gt_ne_st ; auto.
  Qed.
    
End SplaySubCasesLeftGCRotation.
